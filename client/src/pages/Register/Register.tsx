...
const Register: React.FC = () => {
  ...
  return (
    <AuthLayout title="Crear Cuenta">
      <form onSubmit={handleSubmit(onSubmit)} noValidate className='row g-2'>
        <div className="col-sm-6">
            ...
            <input
              id="firstName"
              type="text"
              className={`form-control form-control-sm ${touchedFields.firstName && errors.firstName ? 'is-invalid' : ''}`}
              {...register('firstName')}
              placeholder="Ingresa tu Nombre"
            />
            {touchedFields.firstName && errors.firstName && (
              <div className="invalid-feedback small">{errors.firstName.message}</div>
            )}
          </div>
          ...
          <div className="col-sm-6">
            ...
            <input
              id="lastName"
              type="text"
              className={`form-control form-control-sm ${touchedFields.lastName && errors.lastName ? 'is-invalid' : ''}`}
              {...register('lastName')}
              placeholder="Ingresa tu Apellido"
            />
            {touchedFields.lastName && errors.lastName && (
              <div className="invalid-feedback small">{errors.lastName.message}</div>
            )}
          </div>
        ...
        <div className="col-12">
          ...
          <input
            id="dni"
            type="text"
            className={`form-control form-control-sm ${touchedFields.dni && errors.dni ? 'is-invalid' : ''}`}
            {...register('dni')}
            placeholder="Ingresa tu DNI"
          />
          {touchedFields.dni && errors.dni && (
            <div className="invalid-feedback small">{errors.dni.message}</div>
          )}
        </div>
        ...
        <div className="col-12">
          ...
          <input
            id="email"
            type="email"
            className={`form-control form-control-sm ${touchedFields.email && errors.email ? 'is-invalid' : ''}`}
            {...register('email')}
        ...
        <div className="col-12">
          <label htmlFor="password" className="form-label d-flex align-items-center gap-1">
            Contraseña <span className="text-danger">*</span>
            <div className="position-relative d-inline-block showText" style={{ cursor: 'help' }}>
                <FaExclamationCircle size={14} className="text-muted" />
                <div className="position-absolute bg-dark text-white p-2 rounded shadow-sm hideText" 
                     style={{ 
                       display: 'none', 
                       width: '200px', 
                       fontSize: '0.75rem', 
                     ...
                     role="tooltip">
                  Debe contener 8+ caracteres con mayúscula, minúscula, número y caracter especial (@$!%*?&).
                </div>
            </div>
          </label>
          <input
            id="password"
            type="password"
            className={`form-control form-control-sm ${touchedFields.password && errors.password ? 'is-invalid' : ''}`}
            {...register('password')}
            placeholder="Crea una nueva contraseña"
          />
          {touchedFields.password && errors.password && (
            <div className="invalid-feedback small">{errors.password.message}</div>
          )}
        </div>
        ...
        <div className="col-12">
          ...
          <input
            id="confirmPassword"
            type="password"
            className={`form-control form-control-sm ${touchedFields.confirmPassword && errors.confirmPassword ? 'is-invalid' : ''}`}
